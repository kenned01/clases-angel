...
<section id="Cuerpo_perfil">
		...
		// Empieza funcion de paginacion 
			...
			if (isset($_GET["fecha"])) {
				...
				$consulta ="SELECT * FROM tatu_cita WHERE id_producto='$id' AND status='Pagado' AND fecha='$fecha' ORDER BY hora ASC  LIMIT ".$inicio.", ". $resultados ;

				$consulta2="SELECT * FROM tatu_cita WHERE id_producto='$id' AND status='Pagado' AND fecha='$fecha'   ORDER BY hora ASC"; 
	 
			}

			else
			{
				$consulta ="SELECT * FROM tatu_cita WHERE id_producto='$id' AND status='Pagado' ORDER BY fecha ASC  LIMIT ".$inicio.", ". $resultados ;

				$consulta2="SELECT * FROM tatu_cita WHERE id_producto='$id'  AND status='Pagado'  ORDER BY fecha ASC"; 
			}
														 	...
														 	** <b><?php if ($idioma == "es") { ?>Telefono Cliente<?php } elseif ($idioma == "en") { ?>Customer Phone<?php } ?>:</b> <?php echo $telefono_cliente  ?>  <br><br>
														 	...
														 	<?php 
														 	 ...
														 	 if ($locacion_cita==1) {
														 	 	...
														 	 	<b>La cita se realizara en la direccion del tecnico ?>
