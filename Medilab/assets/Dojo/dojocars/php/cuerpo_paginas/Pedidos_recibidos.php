...
<center>	
	<?php if ($idioma == "es") { ?><h1>Listado de Pedidos Pagados Por Enviar</h1><?php } else if ($idioma == "en"){ ?><h1>List of Paid Orders to Ship</h1><?php } ?>
</center>
...
<center>
 ...
 <form id="form_perfil" name="form_perfil" action="tu_perfil.php" method="GET" enctype="multipart/form-data">
	 ...
	 <?php if ($idioma == "es") { ?>Buscar por Código<?php } else if ($idioma == "en"){ ?>Search Order Code<?php } ?> 
	...
	<input type="hidden" name="op" value="Pedidos_recibidos">
	...
	<center><input style="font-size:1.1rem" type="submit" value="<?php if ($idioma == "es") { ?>Buscar<?php } else if ($idioma == "en"){ ?>Search<?php } ?>" id="boton_generico"></center>
...
<?php 
 ...
 if (isset($_GET["buscador"]))
 {
 	...
 	$consulta="SELECT * FROM pedido WHERE status= 'Pagado' AND  id_pedido= '$codigo_ped' ORDER BY id_pedido DESC LIMIT ".$inicio.", ". $resultados ;

		$consulta2="SELECT * FROM pedido WHERE status= 'Pagado' AND  id_pedido= '$codigo_ped' ORDER BY id_pedido DESC"; 
...

 {
 	...
 	$consulta="SELECT * FROM pedido WHERE status= 'Pagado' ORDER BY id_pedido DESC LIMIT ".$inicio.", ". $resultados ;

		$consulta2="SELECT * FROM pedido WHERE status= 'Pagado' ORDER BY id_pedido DESC"; 
 ...
 echo "<p class='titulo'>";
			...
			{
				  if ($idioma == "es") { echo "No hay resultados a su búsqueda</p>"; } else if ($idioma == "en"){ echo "No results available</p>";  } ;
			}
						...
						<?php if ($idioma == "es") { ?>Dirección:<?php } else if ($idioma == "en"){ ?>Address:<?php } ?> 
									...
									<?php if ($idioma == "es") { ?>Código de área:<?php } else if ($idioma == "en"){ ?>Zip Code: <?php } ?> 
														...
														TOTAL:
														 ...
														 <b><?php echo $registro["pago_total"]  ?> USD.</b><br>
										...
										<?php if ($idioma == "es") { ?>Compañia de Envio<?php } else if ($idioma == "en"){ ?> Shipping Company <?php } ?> 
														...
														<input type="text" required placeholder="Compañia de Envio" name="envio_compania" size="20%"><br><br><br>

														Tracking

														<br><br>

														<input type="text" required placeholder="Traking" name="envio_traking" size="20%"><br><br><br>
...
<script>
  $(document).ready(function() {
    $("#datepicker").datepicker();
														...
														<input type="date" required  name="fecha_envio" size="20%"><br><br><br>
